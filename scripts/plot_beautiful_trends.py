...
class BeautifulTrendsGenerator:
    ...
    def __init__(
        ...
        output_dir: Path = Path("docs/trends"),
    ):
    ...
    def build_comprehensive_timeseries(
        self, dates: List[datetime], snapshots: List[Any]
    ) -> Dict:
    ...
    def plot_repository_evolution_timeline(self, series_data: Dict, top_n: int = 10):
        ...
        top_repos = sorted(
            [
                (name, data["latest_stars"])
                for name, data in series_data.items()
                if data["latest_stars"] > 0
            ],
            key=lambda x: x[1],
            reverse=True,
        )[:top_n]
        ...
        fig.suptitle(
            "🚀 Repository Evolution Timeline - Top Performers",
            fontsize=20,
            fontweight="bold",
            y=0.98,
        )
        ...
        ax1.set_title(
            "⭐ Star Growth Over Time", fontsize=14, fontweight="bold", pad=20
        )
        for i, (repo_name, _) in enumerate(top_repos):
            ...
            if data["dates"] and any(s for s in data["stars"] if s is not None):
                ...
                filtered_dates = [
                    d for d, s in zip(data["dates"], data["stars"]) if s is not None
                ]
        ...
        ax2.set_title(
            "📊 Agentic Index Score Progression", fontsize=14, fontweight="bold", pad=20
        )
        for i, (repo_name, _) in enumerate(top_repos):
            data = series_data[repo_name]
            if data["dates"] and any(
                s for s in data["score"] if s is not None and s > 0
            ):
                filtered_dates = [
                    d
                    for d, s in zip(data["dates"], data["score"])
        ...
        for i, (repo_name, _) in enumerate(top_repos):
            ...
            if data["dates"] and any(f for f in data["forks"] if f is not None):
                filtered_dates = [
                    d for d, f in zip(data["dates"], data["forks"]) if f is not None
                ]
        ...
        for i, (repo_name, _) in enumerate(top_repos):
            ...
            if data["dates"] and any(iss for iss in data["issues"] if iss is not None):
                filtered_dates = [
                    d
                    for d, iss in zip(data["dates"], data["issues"])
                    if iss is not None
                ]
        ...
        plt.savefig(
            self.output_dir / "repository_evolution_timeline.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def plot_current_state_dashboard(self, latest_snapshot: List[Dict]):
        ...
        repos = [
            self.normalize_repo_data(repo)
            for repo in latest_snapshot
            if isinstance(repo, dict)
        ]
        ...
        fig.suptitle(
            "📈 Agentic Index - Current State Dashboard",
            fontsize=24,
            fontweight="bold",
            y=0.98,
        )
        ...
        ax1.set_title(
            "⭐ Stars vs Agentic Score Correlation", fontsize=14, fontweight="bold"
        )
        ax1.set_xscale("log")
        ax1.grid(True, alpha=0.3)
        ...
        bars = ax3.barh(
            ...
            lang_vals,
            color=plt.cm.viridis(np.linspace(0, 1, len(lang_names))),
        )
        ...
        if score_components:
            ...
            ax4.set_xticklabels(
                ["Stars", "Issues", "Recency", "Docs", "Others"], rotation=45
            )
        ...
        ax5.set_title(
            "🏆 Top 10 Repositories by Agentic Index Score",
            fontsize=16,
            fontweight="bold",
            pad=20,
        )
        ...
        plt.savefig(
            self.output_dir / "current_state_dashboard.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def generate_all_charts(self):
        ...
        print(
            f"📊 Found {len(snapshots)} historical snapshots from {dates[0].date()} to {dates[-1].date()}"
        )
