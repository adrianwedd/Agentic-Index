...
class CIIssueCreator:
    ...
    def get_critical_workflows(self) -> List[tuple]:
        ...
        for name, workflow in self.report['workflows'].items():
            ...
            is_critical = (
                workflow['critical_issues'] and (
                    workflow['failure_rate'] > 0.8 or  # >80% failure rate
                    'Recent failure in last 24h' in workflow['critical_issues'] or
                    '3+ consecutive failures' in workflow['critical_issues']
                )
            )
    ...
    def issue_exists(self, workflow_name: str) -> bool:
        ...
        try:
            ...
            if issues:
                print(f"📝 Issue already exists for {workflow_name}: #{issues[0]['number']}")
                return True
            return False
    ...
    def get_failure_analysis(self, workflow_name: str, workflow: Dict) -> str:
        ...
        # Basic stats
        analysis.append(f"**Failure Rate:** {workflow['failure_rate']:.1%} ({workflow['failures']}/{workflow['total_runs']} recent runs)")
        analysis.append(f"**Trend:** {workflow['trend']} {'📉' if workflow['trend'] == 'degrading' else '📈' if workflow['trend'] == 'improving' else '➡️'}")
        
        # Latest run info
        if workflow['latest_run']:
            latest = workflow['latest_run']
            analysis.append(f"**Latest Run:** [{latest['conclusion']}]({latest['url']}) ({latest['age_hours']:.1f}h ago)")
        ...
        if workflow['failure_rate'] > 0.9:
            analysis.append("- 🔥 **URGENT**: Workflow is completely broken - immediate attention required")
        ...
        if '3+ consecutive failures' in workflow['critical_issues']:
            analysis.append("- 🔄 **Pattern**: Multiple consecutive failures suggest systematic issue")
    ...
    def run(self, dry_run: bool = False, max_issues: int = 5) -> None:
        ...
        if dry_run:
            ...
            for workflow_name, workflow in critical_workflows[:max_issues]:
                print(f"  - {workflow_name} ({workflow['failure_rate']:.1%} failure rate)")
            return
        ...
        for workflow_name, workflow in critical_workflows[:max_issues]:
            ...
            if created_count >= 3:
                print(f"⚠️  Rate limit: Created {created_count} issues, stopping to avoid spam")
                break
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Create GitHub issues for critical CI failures")
    parser.add_argument('--report', type=Path, default='web/ci_report.json', 
