...
class TextTrendsGenerator:
    ...
    def create_ascii_bar_chart(
        self, data: Dict[str, int], title: str, max_width: int = 40
    ) -> str:
        ...
        for label, value in sorted(data.items(), key=lambda x: x[1], reverse=True):
            ...
            percentage = (
                (value / sum(data.values())) * 100 if sum(data.values()) > 0 else 0
            )
    ...
    def create_sparkline(self, values: List[float], width: int = 20) -> str:
        ...
        if len(clean_values) > width:
            ...
            sampled_values = clean_values + [clean_values[-1]] * (
                width - len(clean_values)
            )
    ...
    def generate_evolution_report(self, dates: List[datetime], snapshots: List[Any]):
        ...
        top_repos = sorted(
            latest_repos.items(), key=lambda x: x[1]["score"], reverse=True
        )[:10]
            ...
            for date, snap in zip(dates, snapshots):
                for repo in snap:
                    if isinstance(repo, dict):
                        ...
                        if norm_repo["score"] > 0:
                            f.write(
                                f"{date.date()},{norm_repo['name']},{norm_repo['stars']},"
