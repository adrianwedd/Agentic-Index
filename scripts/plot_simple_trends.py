...
class TrendsGenerator:
    ...
    def get_top_repos_evolution(
        self, dates: List[datetime], snapshots: List[Any], top_n: int = 8
    ):
        ...
        top_repos = sorted(latest_scores.items(), key=lambda x: x[1], reverse=True)[
            :top_n
        ]
    ...
    def plot_multi_metric_evolution(self, evolution_data: Dict, repo_names: List[str]):
        ...
        fig.suptitle(
            "🚀 Repository Evolution - Multi-Metric Analysis",
            fontsize=16,
            fontweight="bold",
            y=0.98,
        )
        ...
        for i, repo_name in enumerate(repo_names):
            ...
            # Filter None values
            valid_data = [
                (d, s) for d, s in zip(data["dates"], data["stars"]) if s is not None
            ]
            if valid_data:
        ...
        for i, repo_name in enumerate(repo_names):
            ...
            valid_data = [
                (d, s)
                for d, s in zip(data["dates"], data["scores"])
        ...
        for i, repo_name in enumerate(repo_names):
            data = evolution_data[repo_name]
            valid_data = [
                (d, f) for d, f in zip(data["dates"], data["forks"]) if f is not None
            ]
            if valid_data:
        ...
        plt.savefig(
            self.output_dir / "repository_evolution_comprehensive.png",
            dpi=300,
            bbox_inches="tight",
            facecolor="white",
        )
    ...
    def plot_current_snapshot_analysis(self, latest_snapshot: List[Dict]):
        ...
        fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(16, 10))
        fig.suptitle(
            "📊 Current Repository Landscape Analysis",
            fontsize=16,
            fontweight="bold",
            y=0.98,
        )
        ...
        ax1.set_title(
            "⭐ Stars vs Agentic Score Correlation", fontweight="bold", pad=15
        )
        ...
        plt.subplots_adjust(top=0.93)
        plt.savefig(
            self.output_dir / "current_landscape_analysis.png",
            dpi=300,
            bbox_inches="tight",
            facecolor="white",
        )
    ...
    def generate_summary_report(self, dates: List[datetime], snapshots: List[Any]):
        ...
        latest = snapshots[-1]
        repos = [
            self.normalize_repo_data(repo) for repo in latest if isinstance(repo, dict)
        ]
        repos = [r for r in repos if r["score"] > 0]
        ...
        with open(report_path, "w") as f:
            ...
            if repos:
                ...
                f.write(
                    f"   • Score range: {min(r['score'] for r in repos):.2f} - {max(r['score'] for r in repos):.2f}\n\n"
                )
