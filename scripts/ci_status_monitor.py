...
class CIStatusMonitor:
    ...
    def fetch_ci_runs(self, limit: int = 50) -> None:
        ...
        try:
            cmd = [
                'gh', 'run', 'list', 
                '--limit', str(limit),
                '--json', 'status,conclusion,name,workflowName,url,createdAt,databaseId'
            ]
            ...
            self.runs = [
                ...
                for run in data if run['status'] == 'completed'
            ]
    ...
    def analyze_workflow_health(self) -> None:
        ...
        for workflow_name, runs in workflow_runs.items():
            ...
            recent_failure_rate = sum(1 for r in recent_runs if r.is_failure) / len(recent_runs) if recent_runs else 0
            older_failure_rate = sum(1 for r in older_runs if r.is_failure) / len(older_runs) if older_runs else 0
    ...
    def _summarize_failure(self, errors: List[str]) -> str:
        ...
        patterns = {
            'dependency': ['modulenotfounderror', 'importerror', 'pip install', 'npm install'],
...
"""
    ...
    def _generate_workflow_cards(self) -> str:
        ...
        for workflow in sorted_workflows:
            latest_status = "success" if workflow.latest_run and workflow.latest_run.is_success else "failure"
            
            trend_class = f"trend-{workflow.trend}"
            trend_arrow = {"improving": "📈", "degrading": "📉", "stable": "➡️"}[workflow.trend]
            ...
            if workflow.critical_issues:
                issues_list = "".join(f"<li>{issue}</li>" for issue in workflow.critical_issues)
    ...
    def generate_json_report(self, output_path: Path) -> None:
        ...
        report = {
            ...
            'summary': {
                ...
                'healthy_workflows': sum(1 for w in self.workflows.values() if w.failure_rate < 0.2),
                'total_critical_issues': sum(len(w.critical_issues) for w in self.workflows.values()),
    ...
    def print_console_summary(self) -> None:
        ...
        for workflow in sorted_workflows:
            ...
            trend_emoji = {"improving": "📈", "degrading": "📉", "stable": "➡️"}[workflow.trend]
            
            print(f"{status} {workflow.name:<30} "
...
def main():
    ...
    parser.add_argument('--limit', type=int, default=100, help="Number of runs to analyze")
    parser.add_argument('--output-dir', type=Path, default=Path('.'), help="Output directory")
    parser.add_argument('--format', choices=['console', 'html', 'json', 'all'], 
